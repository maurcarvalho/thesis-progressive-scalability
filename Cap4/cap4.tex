...
Redis + ioredis & Caching layer (L0 vertical optimization) & G3 \\
BullMQ & Async job queues (L1 async decoupling) & G3 \\
...
\subsubsection*{Panel Characterization}

To ensure transparency and allow assessment of panel credibility, Table~\ref{tab:panel-characterization} will be populated after recruitment with anonymized participant profiles. The characterization captures the dimensions most relevant to evaluating architectural guidelines for modular monoliths.

\begin{table}[H]
\centering
\small
\caption{Expert panel characterization (to be populated after recruitment)}
\label{tab:panel-characterization}
\rowcolors{2}{gray!5}{white}
\begin{tabularx}{\linewidth}{p{0.06\linewidth} p{0.12\linewidth} p{0.10\linewidth} X p{0.10\linewidth} p{0.10\linewidth} p{0.10\linewidth}}
\hline
\rowcolor{gray!30}
\textbf{ID} & \textbf{Role} & \textbf{Exp. (yrs)} & \textbf{Domain / Industry} & \textbf{DDD / Modularity} & \textbf{Monolith exp.} & \textbf{MSA exp.} \\
\hline
E1 & \emph{TBD} & -- & -- & -- & -- & -- \\
E2 & \emph{TBD} & -- & -- & -- & -- & -- \\
E3 & \emph{TBD} & -- & -- & -- & -- & -- \\
...
\noindent
Each expert will be assigned an anonymous identifier (E1--E\emph{n}). The characterization dimensions are:
\begin{itemize}[noitemsep,topsep=2pt]
  \item \emph{Role:} Current professional role (e.g., Staff Engineer, PhD Researcher, Engineering Director).
  \item \emph{Experience:} Total years of professional software engineering experience.
  \item \emph{Domain / Industry:} Primary industry sector (e.g., fintech, e-commerce, SaaS, academia) and company size category (startup, mid-size, enterprise).
  \item \emph{DDD / Modularity:} Years of experience with Domain-Driven Design, bounded contexts, or explicit modular architecture practices.
  \item \emph{Monolith experience:} Self-reported experience level with monolithic or modular monolith systems (none / limited / significant / primary).
  \item \emph{MSA experience:} Self-reported experience level with microservices architectures (none / limited / significant / primary).
\end{itemize}

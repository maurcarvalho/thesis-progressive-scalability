...
\subsubsection*{Exercise 1: Cross-Module Internal Reuse ($C_{\mathrm{leak}}\uparrow$, $\rho_{\mathrm{api}}\downarrow$)}
Introduce the most common maintainability anti-pattern: importing another module's internal utility directly instead of interacting through the public surface.

\emph{What you change:} In \texttt{libs/modules/orders/src/features/place-order/service.ts}, add a direct import of an Inventory internal:

\begin{lstlisting}[language=TypeScript,caption={Violation: cross-module internal import},label={lst:g2-ex1-violation}]
// VIOLATION: importing Inventory internals from Orders
import { ProductRepository }
  from '../../../../../../modules/inventory/src/domain/repositories/product.repository';
\end{lstlisting}
...
\emph{Expected signal:} The boundary test that asserts \texttt{ProductRepository} is not accessible from outside Inventory may not catch a deep relative import directly, but \texttt{@nx/enforce-module-boundaries} (if configured) or the grep-based metric check will detect the cross-module reference. Metric impact: $C_{\mathrm{undecl}}$ rises from 0 to 1, $\rho_{\mathrm{api}}$ drops below 1.0, and $C_{\mathrm{leak}}$ increments because an internal symbol is consumed across the boundary.

\emph{Fix:} Remove the direct import. If Orders needs product availability information, it should either (a)~publish an event and let Inventory respond, or (b)~consume a public handler like \texttt{GetProductHandler} through the composition root.

\subsubsection*{Exercise 2: Bidirectional Coupling ($C_{\mathrm{forbid}}\uparrow$, cycle detection)}
Introduce a dependency cycle by having Inventory import from Orders, creating bidirectional coupling.

\emph{What you change:} In \texttt{libs/modules/inventory/src/features/reserve-stock/service.ts}, add:

\begin{lstlisting}[language=TypeScript,caption={Violation: bidirectional coupling},label={lst:g2-ex2-violation}]
// VIOLATION: Inventory now depends on Orders (cycle)
import { GetOrderHandler } from '@tiny-store/modules-orders';
\end{lstlisting}

Since Orders' listeners already consume Inventory events (wired in the composition root), this creates a dependency cycle: Orders $\rightarrow$ Inventory events $\rightarrow$ Orders.

\emph{What you run:}
\begin{lstlisting}[language=bash,caption={Run enforcement after Exercise 2},label={lst:g2-ex2-run}]
npm run test:boundary
npx nx graph  # visualize the dependency graph for cycle detection
\end{lstlisting}

\emph{Expected signal:} If \texttt{@nx/enforce-module-boundaries} is configured with forbidden constraints between peer modules, this import triggers a lint failure. The Nx dependency graph will show a cycle between \texttt{modules-orders} and \texttt{modules-inventory}. Metric impact: $C_{\mathrm{forbid}}\uparrow$ (the import violates the architectural constraint that peer bounded contexts must not import each other directly).

\emph{Fix:} Remove the direct import. If the reserve-stock feature needs order details, introduce an event contract: Inventory publishes a request event, and the composition root wires the response---or restructure the flow so that the necessary data is included in the originating event payload.

\subsubsection*{Exercise 3: API Surface Inflation ($G_{\mathrm{api}}\uparrow$)}
Demonstrate how unnecessary exports increase the public surface, creating downstream obligations and reducing refactoring freedom.

\emph{What you change:} In \texttt{libs/modules/orders/src/index.ts}, add exports of internal symbols:

\begin{lstlisting}[language=TypeScript,caption={Violation: inflating the public surface},label={lst:g2-ex3-violation}]
// VIOLATION: exporting internal types inflates the API surface
...
\emph{Expected signal:} The boundary tests that assert \texttt{(orders as any).Order} is \texttt{undefined} and \texttt{(orders as any).OrderRepository} is \texttt{undefined} now \emph{fail}---these symbols are defined when they should not be. Metric impact: $C_{\mathrm{leak}}$ rises from 0 to 5+ (each newly exposed internal symbol counts), $G_{\mathrm{api}}$ increases by 5 symbols in a single change, and $\rho_{\mathrm{api}}$ degrades as downstream code begins depending on internal types rather than stable handlers.

\emph{Fix:} Remove the internal exports. The public surface should contain only handlers and listener factories---capabilities, not implementation details.
...
0 & Establish baseline & \texttt{test:boundary}\newline\texttt{test:integration} & all pass (baseline) \\
1 & Import Inventory internal from Orders & \texttt{test:boundary} & $C_{\mathrm{undecl}}\uparrow$, $\rho_{\mathrm{api}}\downarrow$ \\
2 & Import Orders handler from Inventory (cycle) & \texttt{test:boundary}\newline\texttt{nx graph} & $C_{\mathrm{forbid}}\uparrow$, cycle \\
3 & Export 5 internal types from Orders \texttt{index.ts} & \texttt{test:boundary} & $C_{\mathrm{leak}}\uparrow$, $G_{\mathrm{api}}\uparrow$ \\
\end{tabularx}
\normalsize

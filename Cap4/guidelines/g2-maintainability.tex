...
\begin{lstlisting}[language=bash,caption={Computing G2 extended metrics},label={lst:g2-extended-metrics}]
# Module Complexity Concentration (Gamma)
# API surfaces: orders=20, inventory=16, payments=7, shipments=8
# max=20, mean=12.75
# Gamma = 20 / 12.75 = 1.57 (well below 3.0 threshold)

# Contract Stability Index (sigma_m)
# Compare current index.ts exports with previous release
# All modules retained their exports across the boundary
# refactoring (handlers/DTOs/events/listeners unchanged)
# sigma_m = 1.0 for all modules (stable baseline)

# Fan-In Concentration (phi_m)
# From the composition root imports:
...
# All cross-module wiring goes through apps/api, not between
# library modules directly. phi_m = 0.0 for all (healthy).
\end{lstlisting}

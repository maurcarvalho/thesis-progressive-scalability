...
The metrics below operationalize modular boundary enforcement as measurable, automated constraints. G1 defines three consolidated metrics that together determine whether module boundaries are intact. Each metric is described in plain language, formally defined for unambiguous implementation, and assigned an enforcement threshold.
...
\midrule
Boundary Violations & Undeclared, forbidden, and event-subscription references across modules & 0 & Yes \\
Encapsulation Violations & Cross-module references that bypass the provider's public API & 0 & Yes \\
Isolation Pass Rate & Module-scoped tests passing with only declared dependencies & 100\% & Yes \\
\bottomrule
\end{tabularx}
\normalsize
...
\medskip
\noindent
\emph{Formal definitions.} The remainder of this section provides the precise definitions used to compute each metric. In prose, metrics are referred to by name; the notation below supports unambiguous tooling implementation.
...
npm run test:boundary
  ...
  \emph{Expected signal:} a boundary test fails, and the encapsulation leakage count increases.
...
npm run test:boundary
  ...
  \emph{Metric impact:} the undeclared or forbidden dependency count increases.
...
\end{lstlisting}
  ...
  \emph{Metric impact:} the isolation pass rate drops and event boundary violations may increase.

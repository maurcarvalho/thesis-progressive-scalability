...
\begin{itemize}[noitemsep,topsep=2pt]
  ...
  \item Dependencies between modules occur only through explicitly declared and verifiable interfaces.
  \item Unintended coupling is prevented, enabling independent evolution of modules within a single deployable system.
\end{itemize}
...
\begin{itemize}[noitemsep,topsep=2pt]
  ...
  \item \emph{Early Verification:} Detect boundary violations at build or test time rather than at runtime.
...
\begin{itemize}[noitemsep,topsep=2pt]
  \item \emph{Convenience-Driven Imports:} Developers import internal classes from another module because the path is shorter or the type is already available. Over time, these imports create a web of hidden coupling that boundary checks cannot detect if enforcement is absent~\cite{parnas1972informationhiding}. This anti-pattern is captured by an increase in $C_{\mathrm{leak}}$ and a decrease in $\rho_{\mathrm{api}}$.
...
\begin{itemize}[noitemsep,topsep=2pt]
  ...
  \item \emph{Circular Dependencies Between Modules:} Module A depends on Module B, which depends on Module A, often through indirect paths that span three or more modules. Circular dependencies make independent testing, reasoning, and extraction impossible: changing any module in the cycle requires understanding all of them~\cite{martin2012clean, yourdon1979structured}. In Tiny Store, the Nx dependency graph visualizes these cycles immediately (\texttt{npx nx graph}). Cycles typically signal that bounded contexts need redrawing or that a shared contract (event, value object) should be extracted into \texttt{libs/shared/}. G1 treats any circular dependency as a blocking violation.
...
\begin{itemize}[noitemsep,topsep=2pt]
  ...
  Number of static references from module $A$ to module $B$ where $A \rightarrow B$ is not declared in the module descriptor.

  \item \emph{Forbidden Dependency Reference Count:}
  Number of static references from module $A$ to module $B$ where $A \rightarrow B$ is explicitly forbidden.

  \item \emph{API-Only Dependency Ratio:}
  For all cross-module references, the proportion that targets the provider module's declared public API surface (interfaces, exported packages, published events) rather than internal packages.

  \item \emph{Encapsulation Leakage Count:}
  Number of external references to internal symbols (classes, packages, resources) that are not part of the provider module's declared public surface.

  \item \emph{Module Isolation Test Pass Rate:}
  Percentage of module-scoped tests that execute with only the module and its declared dependencies available (for example, module-level context loading).
  Failures indicate hidden wiring to undeclared modules, even when imports look compliant.
  
  \item \emph{Event Subscription Boundary Violations (if events are used):}
  Number of event handlers in module $A$ consuming events from module $B$ without a declared $A \rightarrow B$ dependency (or without a declared event subscription, depending on the enforcement model).

 \item \emph{Boundary Bypass Surface Count (optional):}
  Number of occurrences of mechanisms that can bypass static boundaries (for example, reflection-based access, service locator patterns, dynamic class loading) within cross-module interaction paths. This metric is intended as a risk indicator even when no violations are detected statically.
\end{itemize}
...
The foundational argument for modular boundaries traces back to Parnas~\cite{parnas1972informationhiding}, who established that information hiding, rather than functional decomposition, should determine module boundaries. This principle remains the theoretical anchor for G1: modules in a modular monolith must encapsulate implementation details behind stable interfaces, and cross-module access must be mediated through declared contracts rather than direct knowledge of internals.

The systematic literature review conducted in Chapter~3 reveals that modularity is widely acknowledged as a prerequisite for sustainable architecture, yet few works operationalize it as an enforceable property. Abgaz et al.~\cite{abgaz2023decomposition} survey 63 studies on monolith decomposition and find that most approaches focus on \emph{identifying} module boundaries (through clustering, dependency analysis, or domain modeling) but stop short of prescribing mechanisms to \emph{enforce} those boundaries once established. The gap between boundary identification and boundary enforcement is precisely what G1 addresses.

Several SLR-included works provide converging evidence for the importance of explicit modular structure. Montesi and Weber~\cite{montesi2021sliceable} propose the ``sliceable monolith'' concept, arguing that a monolith designed with clean module boundaries can be progressively decomposed into services without a rewrite; their work validates the premise that boundary discipline must precede extraction. Ghemawat et al.~\cite{ghemawat2023towards} describe Google's Service Weaver, which enforces component boundaries at the programming model level and defers deployment topology decisions, reinforcing the principle that modularity is a build-time property, not a deployment-time afterthought. Prakash and Rao~\cite{prakash2024systematic} identify ``module boundary definition'' and ``dependency management'' as two of the top five factors influencing modulith adoption over microservices, based on a systematic analysis of industry and academic sources.

The coupling and cohesion dimensions that G1 operationalizes through metrics ($C_{\mathrm{leak}}$, $C_{\mathrm{undecl}}$, $C_{\mathrm{forbid}}$, $P_{\mathrm{iso}}$) are well grounded in the decomposition literature. Wolfart et al.~\cite{wolfart2021modernizing} present a modernization roadmap where coupling analysis is the first diagnostic step before any migration decision; their roadmap positions boundary enforcement as a precondition for all subsequent architectural evolution. De~Lauretis~\cite{deLauretis2019from} similarly observes that monolithic systems fail during migration attempts not because of inherent architectural limitations but because undeclared dependencies create invisible coupling that only surfaces under extraction pressure. Tsechelidis and Musilek~\cite{tsechelidis2023modular} confirm this finding in their trend analysis, noting that modular monolith adoption is driven primarily by the need to ``control internal coupling before it becomes external complexity.''

Su et al.~\cite{su2024modular} position the modular monolith as a deliberate architectural choice for cloud environments rather than a transitional compromise, and identify enforceable module isolation as the distinguishing characteristic that separates a well-structured monolith from a traditional one. Gravanis and Vatheia~\cite{gravanis2021dont} argue that many systems adopt microservices prematurely and would benefit from enforced modularity within a single deployment unit, a position that directly supports G1's emphasis on boundary discipline as a first-order concern.

Industry experience reports complement these academic findings. Shopify's engineering team documented how their large-scale Rails monolith was restructured around enforced component boundaries to sustain development velocity across hundreds of engineers~\cite{shopify2020monolith, shopify2022}. Jovanovic~\cite{jovanovic2024rewriting} reports similar results from rewriting a legacy system as a modular monolith, noting that automated boundary enforcement was the decisive factor in controlling complexity. These practitioner accounts validate the academic consensus: modularity without enforcement degrades under delivery pressure, and the cost of retroactive boundary repair far exceeds the cost of proactive boundary discipline.
...
npm run test:boundary
  ...
  \DontMark\ Expected signal: a boundary check fails if cross-module imports are constrained by the enforcement suite.
  ...
  Fix (preferred): replace the direct import with event-driven collaboration (publish \texttt{OrderPlaced}, react in Inventory, publish \texttt{InventoryReserved}).
...
\end{lstlisting}
  ...
  Fix: move the subscription back to \texttt{register-listeners.ts}.
\end{itemize}
...
The walkthrough exercises reinforced the practical distinction between the right and wrong
dependency shapes. The right shape is contract-based: modules depend on other modules through
public handlers and event contracts, and subscriptions are registered centrally so that integration is reviewable. The wrong shape is convenience-based: importing another module’s internals, leaking symbols through entry-points, or hiding wiring inside modules. These shortcuts may appear productive in the short term, but they create architectural debt by turning internal implementation details into system-wide dependencies.

Most importantly, the implementation showed how G1 remains enforceable over time. The boundary verification gate (\texttt{npm run test:boundary}), complemented by integration checks (\texttt{npm run test:integration}) and public-surface assertions, turns modularity into a measurable constraint. This makes boundary erosion difficult to introduce accidentally and inexpensive to fix when it occurs. As Tiny Store evolves, the same mechanisms scale with it: new modules can be added, contracts can be expanded deliberately, and the architecture can be kept stable without slowing down delivery. In this sense, G1 is not only a guideline but also the foundation that enables progressive scalability by keeping decomposition options open while the system remains a single deployable unit.

With boundaries enforced and coupling kept explicit, the next constraint shifts from \emph{whether} modules can remain isolated to \emph{how} they can be evolved safely. This transition motivates Guideline G2, which focuses on maintainability: preserving clarity of change, controlling complexity growth, and ensuring that modifications remain localized, testable, and low-risk as the codebase and team scale.
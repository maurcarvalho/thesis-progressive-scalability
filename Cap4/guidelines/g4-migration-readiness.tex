...
\begin{lstlisting}[caption={Extraction configuration manifest},label={lst:g4-extract-config}]
version: '1.0'
...
\begin{lstlisting}[language=Bash,caption={Extract command usage},label={lst:g4-extract-usage}]
...
\begin{lstlisting}[language=Bash,caption={Retract command restoring monolith deployment},label={lst:g4-retract-usage}]
...
\begin{itemize}[noitemsep,topsep=2pt]
  \item \emph{Data Ownership Clarity ($\omega_m$):}
  ...
  \emph{Migration meaning:} prerequisite for schema isolation. \\

  \item \emph{Anti-Corruption Layer Coverage ($\gamma$):}
  The proportion of cross-module integration points (event listeners, API consumers) that include an explicit translation layer:
  \[
  ...
  \emph{Migration meaning:} $\gamma < 1$ indicates integration points where upstream schema changes will break downstream modules. Target: $\gamma = 1$ for extraction candidates. \\

  \item \emph{Saga Completeness ($\sigma$):}
  The proportion of cross-module workflows that have explicitly defined compensating actions for each failure mode:
  \[
    \sigma = \frac{|W_{\mathrm{compensated}}|}{|W|}
  \]
  where $W$ is the set of cross-module workflows involving the candidate module.
  \emph{Migration meaning:} $\sigma < 1$ indicates workflows that rely on transactional guarantees and will exhibit data inconsistency after extraction. \\

  \item \emph{Infrastructure Abstraction Coverage ($\alpha_m$):}
  ...
  \emph{Migration meaning:} $\alpha_m < 1$ means extraction requires code changes in domain or application logic, not just infrastructure wiring. \\

  \item \emph{API Version Coverage ($\nu$):}
  The proportion of public API endpoints and event contracts that carry explicit version metadata:
  \[
    \nu = \frac{|E_{\mathrm{versioned}}|}{|E|}
  \]
  \emph{Migration meaning:} $\nu < 1$ means that after extraction, API evolution will require coordinated deployment of all consumers, eliminating the independent deployment benefit that motivated extraction. \\

  \item \emph{Composite Migration Readiness Score ($\mu_m$):}
  A composite indicator for module $m$:
  \[

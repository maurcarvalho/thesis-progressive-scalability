...
\begin{enumerate}[itemsep=8pt,topsep=2pt]
  ...
  \item \emph{Anti-corruption layers:} Tiny Store includes Anti-Corruption Layer gateways from inception (Listings~\ref{lst:g4-acl-inventory} and~\ref{lst:g4-acl-payments}). The orders module communicates with inventory and payments exclusively through gateway interfaces (\texttt{InventoryGateway}, \texttt{PaymentGateway}). The adapter implementations translate between domain models, so that swapping an in-process call for an HTTP client after extraction requires changing only the adapter, not the domain logic.
...
\begin{itemize}[itemsep=8pt,topsep=2pt]
  ...
  Create a dedicated database schema (or namespace) for orders. Migrate the \texttt{orders} and \texttt{order\_items} tables into this schema. Update the orders repository to use the isolated schema. Verify that no other module queries these tables directly (G3's data ownership metric should increase toward 1.0).
...
// Activities call module APIs through Anti-Corruption Layer gateways.
...
\end{lstlisting}
  ...
  In the inventory module, create a translation layer that maps the \texttt{OrderPlaced} event payload into inventory's own internal command. This way, if orders changes its event schema (e.g., renames a field or restructures the payload), the inventory module's Anti-Corruption Layer absorbs the change without affecting inventory's domain logic.
...
\DoMark & Anti-Corruption Layers in all downstream consumers & Step 4 \\
...
\begin{enumerate}[itemsep=8pt,topsep=2pt]
  ...
  \item \emph{Database Migration:} Create a dedicated PostgreSQL database for the extracted service and migrate data from the shared SQLite database. This step materializes the data ownership principle (G3's data ownership = 1.0) by providing the module with an isolated persistence layer.

  \item \emph{Service Scaffolding:} Generate a complete Express-based service application with Drizzle ORM configuration, health check endpoints, and integration with the existing module handlers. The scaffolded service uses the same Temporal workflow definitions and Kafka event publishers as the monolith, ensuring behavioral equivalence.
...
\begin{itemize}[itemsep=8pt,topsep=2pt]
  \item \textbf{Deferred Readiness}\\
  Postponing migration preparation until extraction is imminent. At that point, the accumulated coupling (shared tables, implicit transactions, unversioned APIs) makes extraction a multi-month project instead of a multi-day configuration change.
  \emph{Approach:} treat readiness as a continuous investment; apply G4 steps incrementally from the first sprint.

  \item \textbf{Shared Database as Hidden Integration}\\
  ...
  \item \textbf{Cross-Module Transactions Masking Failure Modes}\\
  Relying on database transactions to maintain consistency across modules hides the failure modes that will surface after extraction. When the inventory reservation and the order creation are in the same transaction, partial failure is impossible. After extraction, partial failure is the default. Teams that have not designed compensating actions discover this in production.
  \emph{Approach:} introduce saga orchestration inside the monolith before extraction, forcing the team to confront failure modes early.

  \item \textbf{Payload Coupling Without Anti-Corruption Layers}\\
  When downstream modules consume event payloads directly without translation, any change to the upstream module's event schema breaks all consumers simultaneously. This creates a coordination bottleneck that is incompatible with independent deployment.
  \emph{Approach:} place an Anti-Corruption Layer at each integration point to translate between domain models.

  \item \textbf{Premature Extraction Without Readiness}\\
  Extracting a module that has not been prepared (shared state, no Anti-Corruption Layers, no saga) creates a distributed monolith: the operational complexity of microservices with the coupling characteristics of a monolith. This is strictly worse than the original state~\cite{gravanis2021dont}.
  \emph{Approach:} require Extraction Readiness ($\varepsilon_m$) $\geq 0.8$ and all G4 preparation steps complete before initiating extraction.
\end{itemize}
...
\begin{itemize}[itemsep=8pt,topsep=2pt]
  \item \textbf{Data Ownership (per module):}
  ...
  Reused from G3, scoped to the candidate module. A value of 1.0 means the module owns all its tables exclusively. A value below 1.0 indicates shared tables that block extraction.
  \[
    \omega_m = \frac{|T_{\mathrm{module\text{-}only}}|}{|T_m|}
  \]
  \emph{Threshold:} complete data ownership ($\omega_m = 1$) before extraction. \\

  \item \textbf{Anti-Corruption Layer Coverage:}
  ...
  \]
  where $I$ is the set of all cross-module integration points and $I_{\mathrm{ACL}}$ is the subset mediated by an Anti-Corruption Layer.
  \emph{Target:} all integration points protected by Anti-Corruption Layers ($\gamma = 1$). \\

  \item \textbf{Saga Completeness:}
  ...
  \emph{Target:} all workflows have compensating actions defined ($\sigma = 1$). \\

  \item \textbf{Abstraction Coverage (per module):}
  ...
  \]
  \emph{Threshold:} all infrastructure access through ports ($\alpha_m = 1$). \\
  ...
  \]
  \emph{Target:} all public APIs carry version metadata ($\nu = 1$). \\
...
\begin{itemize}[itemsep=8pt,topsep=2pt]
  ...
  Maintain a registry of all published event contracts, including version history, schema, and consuming modules. This registry serves as the source of truth for Anti-Corruption Layer design and for assessing the impact of schema changes.
\end{itemize}
...
Anti-Corruption Layer in place & Translation layer at boundary & Direct model sharing \\
...
Migration from monoliths to microservices is one of the most extensively studied topics in modern software architecture. Research on migration intentions, strategies, and challenges consistently shows that teams underestimate the coupling embedded in shared databases and implicit co-location assumptions~\cite{fritzsch2019}. Comprehensive reviews of decomposition techniques observe that most approaches focus on identifying service candidates rather than on preparing the monolith for extraction~\cite{abgaz2023decomposition}. Existing modernization roadmaps acknowledge that proactive readiness, designing for extraction before the need arises, remains underexplored~\cite{wolfart2021modernizing}. Incremental transition strategies emphasize that the migration path should be planned from the outset rather than improvised under pressure~\cite{bozan2020transition}, and surveys of migration benefits and challenges confirm that hidden coupling and shared state are the dominant cost drivers~\cite{salii2023migrating}. The evolution of microservices identification techniques in monolith decomposition~\cite{oumoussa2024evolution}, dynamic decomposition approaches~\cite{nassima2024dynamic}, and cloud modernization strategies~\cite{griffin2022moving} all reinforce the need for infrastructure readiness before extraction.

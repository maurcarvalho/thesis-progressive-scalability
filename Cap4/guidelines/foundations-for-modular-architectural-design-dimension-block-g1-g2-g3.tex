...
This risk connects to the \emph{model--code gap}~\cite{fairbanks2010justenough}: the divergence between architecture as documented and architecture as implemented. Fairbanks proposes an \emph{architecturally-evident coding style}, where decisions are encoded directly into module boundaries and enforced dependency rules. Mechanisms include explicit public interfaces, restricted import paths, and structural checks that fail when constraints are violated. The central implication is that modular boundaries should be explicit, inspectable, and continuously enforced.
...
\begin{lstlisting}[basicstyle=\ttfamily\small,frame=single,framerule=0.3pt,xleftmargin=0.5cm,xrightmargin=0.5cm]
modules/
  orders/

...
\begin{lstlisting}[language=bash,caption={Extracted orders service Dockerfile (D2)},label={lst:g5-dockerfile-d2}]
...
Note that the Docker Compose configuration in Listing~\ref{lst:g5-docker-compose} already includes the \texttt{orders-service} entry alongside the monolith, reflecting Tiny Store's design principle that both deployment topologies (D0 and D1) are supported from inception. The \texttt{orders-service} connects to a dedicated database schema (\texttt{tinystore\_orders}), completing the schema isolation from G4. The \texttt{api} service routes order-related requests to \texttt{orders-service} via HTTP through the \texttt{ORDERS\_SERVICE\_URL} environment variable. This routing change is confined to the composition root; no domain logic changes.


\subsubsection*{Kamal: Production Deployment Without Kubernetes}

While Docker Compose serves local development and CI, production deployment requires zero-downtime deploys, SSL termination, and infrastructure provisioning. Tiny Store uses Kamal\footnote{\url{https://kamal-deploy.org}}, a deployment tool from Basecamp that provisions Docker containers on bare servers via SSH---without requiring Kubernetes. Kamal aligns with G3's progressive scalability philosophy: it provides production-grade deployment at D0 complexity, and supports D1$\to$D2 progression by adding service entries to the same configuration file.

\begin{lstlisting}[language=bash,caption={Kamal deployment configuration (\texttt{config/deploy.yml})},label={lst:g5-kamal-deploy}]
...
\begin{itemize}[noitemsep,topsep=2pt]
  \item \emph{D0 with full infrastructure:} A single \texttt{kamal setup} command provisions Postgres, Redis, Kafka, Temporal, and Jaeger as ``accessories'' alongside the application container. The entire production-grade stack from Table~\ref{tab:tech-stack} is deployed with one command.
  \item \emph{D0$\to$D1 progression:} When the orders module is extracted, a second \texttt{servers} entry is added for \texttt{orders-service} with its own Dockerfile and dedicated database. Kamal deploys both containers independently with zero-downtime rolling updates. No Kubernetes manifests, no Helm charts, no cluster management.
  \item \emph{Contrast with Kubernetes:} Kubernetes provides powerful orchestration but introduces significant operational complexity (control plane management, YAML manifests, ingress controllers, service meshes). For a startup at D0--D1, Kamal provides the essential deployment properties---zero-downtime deploys, SSL via Let's Encrypt, health checks, rollback---at a fraction of the operational overhead. This aligns with G3's principle of proportional intervention: Kubernetes becomes appropriate at D2+ when multiple services require service discovery and autoscaling.
  \item \emph{Rollback:} \texttt{kamal rollback} reverts to the previous container image instantly, making deployment decisions reversible.
\end{itemize}
...
\paragraph{D1: Adding the extracted orders service.} A separate Kamal destination configuration (\texttt{config/deploy.orders-service.yml}) deploys the orders service alongside the monolith. Traefik, Kamal's built-in reverse proxy, routes requests based on path prefix:

\begin{lstlisting}[language=bash,caption={Kamal D1 configuration for orders service ({\ttfamily config/deploy.orders-service.yml}, trimmed)},label={lst:g5-kamal-d1}]
...
\paragraph{Deployment scripts.} Two shell scripts operationalize the D0 and D1 topologies:
...
\begin{itemize}[noitemsep,topsep=2pt]
  ...
  \item \texttt{bin/deploy-d1} --- Deploys both the monolith and the orders service. Executes \texttt{kamal deploy} followed by \texttt{kamal deploy -d orders-service}.
\end{itemize}
...
\begin{itemize}[noitemsep,topsep=2pt]
  \item \emph{CI Scope Efficiency ($\eta_{\mathrm{CI}}$):}
  The ratio of modules tested to modules changed, measuring how effectively the pipeline scopes work to affected modules:
  \[
    \eta_{\mathrm{CI}} = \frac{|M_{\mathrm{changed}}| + |M_{\mathrm{transitively\_affected}}|}{|M_{\mathrm{tested}}|}
  \]
  \emph{Deployment meaning:} $\eta_{\mathrm{CI}} < 1$ indicates the pipeline tests modules that are not affected by the change (wasted work). $\eta_{\mathrm{CI}} = 1$ means the pipeline scope is optimal. \\

  \item \emph{Build Time per Affected Scope ($T_{\mathrm{build}}$):}
  The wall-clock time to build and test only the affected modules for a given change.
  \emph{Deployment meaning:} sustained increase in $T_{\mathrm{build}}$ for small changes indicates pipeline inefficiency or module dependency bloat. \\

  \item \emph{Deployment Artifact Count ($N_{\mathrm{artifacts}}$):}
  The number of distinct deployment artifacts (Docker images) produced by the pipeline.
  \emph{Deployment meaning:} $N_{\mathrm{artifacts}} = 1$ at D0; $N_{\mathrm{artifacts}} > 1$ at D2. Growth should track G3 Level L3 extractions. \\

  \item \emph{Infrastructure Parity Score ($\pi$):}
  The proportion of production infrastructure dependencies (Kafka, Temporal, databases) that are also present in the local development environment (Docker Compose):
  \[
    \pi = \frac{|I_{\mathrm{local}}|}{|I_{\mathrm{production}}|}
  \]
  \emph{Deployment meaning:} $\pi < 1$ indicates environment drift that increases the risk of deployment failures. Target: $\pi = 1$. \\

  \item \emph{Independent Deployment Rate ($\delta$):}
  The proportion of deployments that affect only a single deployment target (no coordinated multi-target releases):
  \[
    \delta = \frac{|D_{\mathrm{independent}}|}{|D|}
  \]
  \emph{Deployment meaning:} low $\delta$ after extraction indicates that modules are not truly independently deployable, suggesting residual coupling in the deployment pipeline or in shared state. \\

  \item \emph{Rollback Success Rate ($R$):}
  The proportion of rollback attempts that succeed without requiring coordinated rollbacks of other targets.
  \emph{Deployment meaning:} low $R$ indicates deployment coupling that G5 is designed to prevent.
\end{itemize}
...
\begin{itemize}[noitemsep,topsep=2pt]
  ...
  \item \texttt{config/deploy.orders-service.yml} --- Kamal D1 configuration (extracted orders)
  \item \texttt{bin/deploy} --- D0 deployment script
  \item \texttt{bin/deploy-d1} --- D1 deployment script (monolith + orders service)

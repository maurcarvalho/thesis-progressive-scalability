...
\begin{itemize}[itemsep=8pt,topsep=2pt]
  \item \textbf{Repository Splitting During Extraction}\\
  ...
  \item \textbf{Build Everything CI}\\
  Running all tests and builds on every commit regardless of what changed. This wastes CI resources, slows feedback loops, and creates incentives to skip tests.
  \emph{Approach:} use module-aware CI (D1) to build and test only affected modules.

  \item \textbf{Infrastructure Bolted On During Extraction}\\
  Adding Kafka, Temporal, or database provisioning only when a module is extracted. This creates a ``big bang'' infrastructure change that compounds the risk of the extraction itself.
  \emph{Approach:} require infrastructure-as-code from day one (D0), so extraction changes only configuration.

  \item \textbf{Monolithic Dockerfile for a Modular System}\\
  ...
  \emph{Approach:} use per-target Dockerfiles that copy only the modules needed for each deployment unit.

  \item \textbf{Coordinated Deployments After Extraction}\\
  Requiring all deployment targets to be released simultaneously. This negates the independent deployment benefit that motivated extraction.
  \emph{Approach:} each target has its own release pipeline, triggered by changes to its affected scope.
\end{itemize}
...
\begin{itemize}[itemsep=8pt,topsep=2pt]
  \item \textbf{CI Scope Efficiency:}
  ...
  \]
  \emph{Target:} perfect scope efficiency, testing only affected modules ($\eta_{\mathrm{CI}} = 1$). \\
  ...
  \item \textbf{Infrastructure Parity:}
  ...
  \]
  \emph{Target:} complete development-production infrastructure parity ($\pi = 1$). \\

  \item \textbf{Independent Deployment Rate:}
  ...
  \]
  \emph{Target:} majority of deployments are independent after extraction ($\delta \geq 0.8$). \\
...
\noindent
\emph{Verification strategy:} G5 metrics are tracked from D0 onward. Even before extraction, CI scope efficiency and build time provide early signals about pipeline efficiency. After extraction (D2), independent deployment rate and rollback success verify that independent deployment is genuinely achievable. A declining independent deployment rate after extraction indicates that the deployment strategy has not fully decoupled from the monolith, requiring investigation of shared infrastructure or coordinated release dependencies.

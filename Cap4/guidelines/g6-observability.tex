...
Module-scoped observability also supplies the differential debugging affordance of a modular monolith: a single session can be followed through correlation identifiers as it traverses multiple modules, surfacing the cross-module interaction that produced an error or a latency spike. Per-module metrics indicate \emph{that} a module is slow; the propagated trace shows \emph{where} the slowness originates and how it ripples across module boundaries. With cross-domain traceability in place, a modular monolith is debuggable at the same level of granularity as a distributed system, without the operational cost of a distributed observability stack.
...
\begin{itemize}[itemsep=8pt,topsep=2pt]
  \item \emph{Aggregate-Only Monitoring (Metric Coverage below 100\%):}
  ...
  \item \emph{Instrumentation After Extraction (Baseline Coverage = 0):}
  ...
  \item \emph{Log-Only Observability (Trace Completeness = 0):}
  ...
  \item \emph{Custom Observability Frameworks (vendor lock-in risk):}
  ...
  \item \emph{Alert Fatigue from Application-Level Thresholds (Alert Precision drops):}
...
\subsection*{The Observability Deficit}
\label{sec:g6-observability-deficit}

Monoliths are treated as observability black boxes. Application-level monitoring reports aggregate metrics (total latency, overall error rate) but cannot attribute performance characteristics to individual modules. Without module-level visibility, G3's scalability triggers cannot be evaluated (``which module needs scaling?''), G4's readiness assessments have no evidence base (``is this module ready for extraction?''), and G5's deployment decisions are made on intuition rather than data.

Microservices close this deficit by default: each service has its own metrics endpoint, log stream, and trace context. In a monolith, this visibility must be deliberately constructed. The Observability Deficit widens as the system grows: more modules produce more aggregate noise, making it progressively harder to isolate the source of performance degradation. G6 closes the deficit by scoping instrumentation to module boundaries, creating the same observability posture inside the monolith that microservices achieve through deployment isolation.
...
\begin{enumerate}[itemsep=8pt,topsep=2pt]
  \item \emph{The Observability Deficit as a formal concept.} G6 names the phenomenon where a modular monolith has clean boundaries and extraction readiness but lacks module-level visibility to evaluate scaling triggers, readiness assessments, and deployment decisions (Section~\ref{sec:g6-observability-deficit}), analogous to G5's Deployment-Architecture Mismatch and G4's Migration Readiness Gap.
  ...
  \item \emph{Cross-domain traceability as the differential debugging affordance.} Module-level metrics indicate which module degrades; the trace propagated through correlation identifiers binds together cross-module interactions into a single debugging artifact, surfacing where in the cross-domain path the degradation originated. This affordance distinguishes module-scoped observability from per-module monitoring in isolation and makes the modular monolith a transparent debuggable system, not a federation of metric endpoints.
...
G6 is demonstrated through the Tiny Store observability infrastructure~\cite{tinystore2026}, which produces module-scoped logs, metrics, and traces from day one. The tutorial walks through four observability pillars: structured logging, module-scoped metrics, distributed tracing, and alert configuration. Each step is reproducible on any modular monolith with handler-based module boundaries.
...
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize,language=TypeScript,caption={Module-scoped structured logger},label={lst:g6-logger}]
...
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize,language=TypeScript,caption={Module-scoped metrics factory},label={lst:g6-metrics}]
...
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize,language=TypeScript,caption={Traced handler with metrics integration},label={lst:g6-traced-handler}]
...
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize,language=yaml,caption={Module-scoped Prometheus alert rule},label={lst:g6-alert}]
- alert: HighErrorRate
  expr: |
...
\emph{Resolution:} replace with \texttt{getModuleLogger('orders').info(...)}.
...
\emph{Resolution:} wrap the handler in \texttt{tracer.startActiveSpan()} and record metrics in the \texttt{finally} block.

\subsubsection*{Exercise 3: Broken Trace Propagation (Trace Completeness $\downarrow$)}

The event publisher does not inject trace context into message headers. Downstream listeners start new traces instead of continuing the existing one. The order fulfillment flow (OrderPlaced $\to$ InventoryReserved $\to$ PaymentProcessed $\to$ ShipmentCreated) shows as four disconnected traces in the trace viewer instead of one unified trace.\\
\emph{Resolution:} inject trace context via \texttt{injectTraceContext(\{\})} in every \texttt{eventPublisher.publish()} call. Consumers extract the trace context from message headers and continue the parent span, producing a single end-to-end trace.
...
\begin{itemize}[itemsep=2pt,topsep=2pt]
  \item \textbf{Ex.\,1:} Missing module context in logs → Log Attribution Rate drops.
  \item \textbf{Ex.\,2:} Uninstrumented handler → Metric Coverage drops.
  \item \textbf{Ex.\,3:} Broken trace propagation → Trace Completeness drops.
\end{itemize}

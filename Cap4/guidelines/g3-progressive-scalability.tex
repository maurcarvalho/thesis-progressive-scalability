...
These anti-patterns emerge when scaling decisions are driven by anticipation rather than evidence. G3 addresses them by embedding infrastructure abstractions from inception, tracking scalability metrics longitudinally, and requiring evidence-based justification for each spectrum-level transition.
...
Tiny Store includes a caching layer from day one, not as an optimization added under pressure. The \texttt{CacheService} enforces module-namespaced keys through a \texttt{buildKey()} method, ensuring that cache entries are isolated per bounded context and can be invalidated independently. The service uses an adapter pattern: an \texttt{InMemoryCacheAdapter} serves development and testing, while a \texttt{RedisCacheAdapter} is injected for production workloads.

\begin{lstlisting}[language=TypeScript,caption={CacheService with module-namespaced keys and adapter pattern (cache.service.ts)},label={lst:cache-service}]
class CacheService {
...
\begin{lstlisting}[language=TypeScript,caption={QueueService with module-scoped queues and adapter pattern (queue.service.ts)},label={lst:queue-service}]
class QueueService {
...
\noindent
Listing~\ref{lst:label-job} shows a concrete use case: shipment label generation is enqueued asynchronously with exponential backoff, decoupling the critical path (shipment creation) from the non-critical side effect (carrier API call for label generation).

\begin{lstlisting}[language=TypeScript,caption={Shipment label generation job with async decoupling (generate-label.job.ts)},label={lst:label-job}]
...
\noindent
Because the queue infrastructure is present from day one, the team can introduce async decoupling for any inter-module interaction by defining a new queue; no infrastructure migration is required.
...
\begin{enumerate}[itemsep=4pt,topsep=2pt]
  ...
  \item The event bus switches from in-process dispatch to a Kafka topic, using the same event contracts defined since inception.
...
The remainder of this section is organized as a hands-on tutorial. The Reader Map orients the practitioner through the exercise sequence. The Exercise Walkthrough introduces three controlled violations, each targeting a specific G3 metric ($\rho_{\mathrm{sync}}$, $\omega$, $\alpha$). Each exercise follows the same structure: introduce a single change, run the G3 gate, observe the metric regression, and apply the fix. The exercises assume the G1 and G2 baselines are passing and takes approximately 20 minutes.

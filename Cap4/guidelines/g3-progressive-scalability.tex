...
\begin{itemize}[itemsep=8pt,topsep=2pt]
  ...
  Extracting modules into services before exhausting in-process optimizations. This introduces distributed-system complexity (network latency, partial failures, eventual consistency) without evidence that distribution is necessary. The cost is disproportionate to the benefit, particularly for early-stage teams with limited operational maturity~\cite{gravanis2021dont, su2024from}. \textbf{Metric signal:} detectable when extraction readiness thresholds are used as triggers before L0--L2 interventions have been exhausted; a premature L3 decision despite low throughput divergence confirms this anti-pattern.

...
\subsubsection*{G3 Applied: Progressive Scalability in the Tiny Store}
...
// Singleton; module-namespaced Redis client
...
// Singleton; manages module-scoped BullMQ queues
...
services:
  ...
  # Remaining modules continue in the monolith
  monolith:
    build:
      context: .
...
\paragraph*{G3 Metrics: Tiny Store Baseline}
...
\emph{Cross-Module Synchronous Call Ratio ($\rho_{\mathrm{sync}}$):} All inter-module interactions in the composition root flow through \texttt{eventBus.subscribe}, with zero synchronous cross-module calls at the library level. Handler instantiations in \texttt{register-listeners.ts} serve event callbacks, not direct synchronous invocations.
...
To keep the summary compact, the table uses the G3 metric notation introduced earlier. $\rho_{\mathrm{sync}}$ denotes the cross-module synchronous call ratio (lower is better), $\omega$ denotes the data ownership clarity index (1.0 = perfect), and $\alpha$ denotes infrastructure abstraction coverage (1.0 = perfect).

\paragraph*{Exercise summary}
\small
\noindent
...
\begin{itemize}[noitemsep,topsep=2pt]
  ...
  \item \emph{Exercise 0: Establish the G3 Scalability Baseline.}
...
# All green: rho_sync=0.0, omega=1.0, alpha=1.0, epsilon>=0.8
  ...
  \newline\item \emph{Exercise 1: Synchronous Cross-Module Call ($\rho_{\mathrm{sync}}\uparrow$).}
...
npm run test:scalability
  ...
  Expected signal: the $\rho_{\mathrm{sync}}$ test fails. The scalability gate detects a direct import from \texttt{@tiny-store/modules-payments} inside the shipments module source. $\rho_{\mathrm{sync}}$ rises from 0.0 because there is now a synchronous cross-module interaction alongside the 23 async subscriptions.\newline
  ...
  \newline\item \emph{Exercise 2: Shared Table Access ($\omega\downarrow$).}
...
/**
 * (*@\color{red}{$\times$}@*) VIOLATION: orders reads inventory's 'products' table directly.
 */
...
npm run test:scalability
  ...
  Expected signal: the $\omega$ test fails. The data ownership check detects that the \texttt{products} table (owned by inventory) is referenced via SQL in the orders module source. $\omega$ drops below 1.0 because one entity now has two accessing modules.\newline
  Metric impact: $\omega\downarrow$, $\varepsilon_{\mathrm{orders}}\downarrow$.\newline
  Fix: expose stock availability through an event (\texttt{InventoryUpdated} with current stock levels) or a query handler in the inventory module, consumed through the composition root. The orders module should never reference another module's tables.\newline
  Reference file: \texttt{exercises/g3/ex2-shared-inventory-view.ts}\newline

  \newline\item \emph{Exercise 3: Direct Infrastructure Dependency ($\alpha\downarrow$).}
 ...
 * Module imports ioredis instead of @tiny-store/shared-infrastructure.
 */
...
npm run test:scalability
  ...
  Expected signal: the $\alpha$ test fails. The infrastructure abstraction check detects a direct import of \texttt{ioredis} in the inventory module source. $\alpha$ drops below 1.0 because one module now has a hard-coded infrastructure dependency.\newline
  Metric impact: $\alpha\downarrow$, $\varepsilon_{\mathrm{inventory}}\downarrow$.\newline
  Fix: use a \texttt{CacheService} from \texttt{@tiny-store/shared-infrastructure} instead of importing Redis directly. This ensures that infrastructure configuration is injectable and the module can be extracted without carrying hard-coded connection details.\newline
...
\begin{itemize}[itemsep=8pt,topsep=2pt]
  ...
  Composite metric synthesizing boundary health (G1), maintainability (G2), and scalability preparation (G3) to quantify how prepared a module is for independent deployment. A high score indicates extraction would be a low-risk topology change; a low score reveals preparation debt~\cite{wolfart2021modernizing}. The weights are derived from the reference implementation's scoring dimensions (\texttt{extraction-readiness.ts}):
  \[

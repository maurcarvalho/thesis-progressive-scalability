...
\noindent
Each module operates within its own PostgreSQL schema from the initial \texttt{docker-compose up}. Cross-module data access is mediated through published contracts (events or API calls), never through direct table queries. Because the repository adapter is injected at composition time, rolling back from L2 to a shared database requires only rebinding the adapter. The module's tables remain in their dedicated schema, preserving isolation even if the dedicated instance is decommissioned. When L3 extraction occurs, the module's schema migrates to a dedicated database instance, which is a deployment topology change rather than a data migration.
...
\noindent
Why G1 and G2 pass: there is no import from \texttt{@tiny-store/modules-inventory}. The query is raw SQL executed through the DataSource, and no boundary test detects cross-module table access. G2 sees no change in module-level dependencies.

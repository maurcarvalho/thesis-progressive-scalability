...
\noindent
The database configuration creates all module schemas on startup and provides a single shared connection. Because schema ownership is declared in the table definition itself, there is no need for per-module connection management or runtime schema switching:

\begin{lstlisting}[language=TypeScript,caption={Database configuration with declarative schema creation (database.config.ts)},label={lst:g3-db-config}]
...
const MODULE_SCHEMAS =
  ['inventory', 'orders', 'payments', 'shipments'] as const;

export const createDatabaseConnection =
  async (): Promise<DrizzleDb> => {
  const sql = postgres({ /* connection config from env */ });
  for (const schema of MODULE_SCHEMAS) {
    await sql.unsafe(
      `CREATE SCHEMA IF NOT EXISTS "${schema}"`);
  }
  // CREATE TABLE IF NOT EXISTS for each module's tables
  return drizzle(sql);
};
\end{lstlisting}

\noindent
Each module operates within its own PostgreSQL schema from the initial \texttt{docker-compose up}. Cross-module data access is mediated through published contracts (events or API calls), never through direct table queries. Because the table definitions use \texttt{pgSchema()}, Drizzle automatically qualifies all queries with the correct schema prefix (e.g., \texttt{inventory.products}), making cross-schema access impossible through the ORM layer. When L3 extraction occurs, the extracted module simply changes its connection string to point at a dedicated database instance; the \texttt{pgSchema()} declarations, repository implementations, and domain logic remain unchanged. This is a deployment topology change rather than a data migration.

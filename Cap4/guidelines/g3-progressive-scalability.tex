...
\subsubsection*{L1: Module-Namespaced Caching}

The \texttt{CacheService} provides a Redis-backed caching layer with module-namespaced keys, ensuring that cache entries are isolated per bounded context. The \texttt{buildKey} method constructs keys using the pattern \texttt{prefix:module:key}, preventing cross-module cache collisions and enabling per-module cache invalidation.

\begin{lstlisting}[language=TypeScript,caption={CacheService with module-namespaced keys (L1 vertical optimization)},label={lst:g3-cache-service}]
...
Every cache call requires the module name as the first argument, making module attribution automatic. When the orders module caches a product lookup, the key becomes \texttt{tiny-store:orders:product:123}, fully isolated from inventory's \texttt{tiny-store:inventory:product:123}.

\subsubsection*{L2 Async: Module-Scoped Queues with BullMQ}

The \texttt{QueueService} creates module-scoped BullMQ queues following the convention \texttt{module:purpose}. This design establishes asynchronous decoupling as an architectural requirement from day one, not a migration step added when load increases.

\begin{lstlisting}[language=TypeScript,caption={QueueService with module-scoped queues (L2 async decoupling)},label={lst:g3-queue-service}]
...
\begin{lstlisting}[language=TypeScript,caption={Order confirmation email job with async decoupling},label={lst:g3-email-job}]
...
\subsubsection*{L2 Data: Per-Module Schema Isolation}

Schema isolation is built into the monolith from project inception. Each module receives its own PostgreSQL schema, and the \texttt{getModuleConnection} function returns a \texttt{DataSource} scoped to that schema via \texttt{search\_path}. This makes data ownership ($\omega = 1$) an enforceable property rather than an aspiration.

\begin{lstlisting}[language=TypeScript,caption={Database schema isolation per module (L2 data isolation)},label={lst:g3-schema-isolation}]
...
\subsubsection*{Extraction Readiness Scoring}
...
\begin{lstlisting}[language=TypeScript,caption={Extraction readiness score calculator (composite metric)},label={lst:g3-readiness-score}]
...
//   Event-driven comm.    [@@@@@@@@@@] 20/20  Events: Y, Listeners: Y
//   ACL layer             [@@@@@@@@@@] 15/15  ACL adapter found
//   Database schema       [@@@@@@@@@@] 20/20  Schema 'orders' configured
//   Versioned events      [@@@@@@@@@@] 20/20  Versioned events found
...
\begin{itemize}[noitemsep,topsep=2pt]
  \item \texttt{libs/shared/infrastructure/src/cache/cache.service.ts} --- CacheService with module-namespaced keys (Listing~\ref{lst:g3-cache-service})
  \item \texttt{libs/shared/infrastructure/src/queue/queue.service.ts} --- QueueService with module-scoped queues (Listing~\ref{lst:g3-queue-service})
  \item \texttt{libs/modules/orders/src/jobs/order-confirmation-email.job.ts} --- Async job example (Listing~\ref{lst:g3-email-job})
  \item \texttt{libs/shared/infrastructure/src/database/schema-isolation.ts} --- Per-module schema isolation (Listing~\ref{lst:g3-schema-isolation})
  \item \texttt{tools/metrics/extraction-readiness.ts} --- Extraction readiness scoring (Listing~\ref{lst:g3-readiness-score})
\end{itemize}

% TODO: Consider adding a reference to the Amazon Prime Video case study \cite{primevideo2023}
% as an industry example of moving from microservices back to a monolith for cost and
% performance reasons, reinforcing that distribution is not always the answer.

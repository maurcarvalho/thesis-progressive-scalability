...
\begin{itemize}[itemsep=8pt,topsep=2pt]
  ...
  \begin{itemize}[itemsep=2pt,topsep=2pt]
    ...
    \item $a_m$ is \emph{Anti-Corruption Layer coverage}: the proportion of cross-module interactions mediated through anti-corruption layers (from G2);
  ...
  Boundary compliance receives the highest weight (0.25) because undeclared cross-module imports create hard extraction blockers that require code changes in both the extracted module and its consumers. Async readiness and data ownership share the second tier (0.20 each) because both represent infrastructure-level coupling that, if unresolved, forces coordinated migration across multiple modules. Event contract versioning (0.20) ensures that extracted modules can communicate through stable schemas without tight temporal coupling. Anti-Corruption Layer coverage receives the lowest weight (0.15) because anti-corruption layers, while valuable for translation isolation, can be introduced incrementally during extraction without blocking the process. These weights are proposed defaults derived from the relative extraction risk each dimension represents; teams should calibrate them based on their system's specific coupling profile and extraction priorities. The validation of these defaults through practitioner interviews is part of the verification plan outlined in Chapter~5.
\end{itemize}
...
The same adapter pattern applies to the event bus itself. At L0, events are dispatched in-process through the \texttt{EventBus} singleton. When a module pair triggers the L1 threshold, the event bus switches from in-process dispatch to a Kafka topic, using the same event contracts defined at project start. The caller publishes an event; the adapter decides whether it is dispatched in-memory or written to a Kafka partition. Because the queue and event bus infrastructure are available from the start, the team can introduce async decoupling for any inter-module interaction by swapping an adapter. No infrastructure migration is required.
...
\noindent
In production, the monolith and the extracted service communicate through Kafka topics rather than the in-process event bus. The event contracts (payload schemas, version fields, aggregate IDs) remain identical; only the transport changes. Three infrastructure changes occur at extraction, none of which touch domain logic:
...
const checks: CheckResult[] = [
  ...
  checkAntiCorruptionLayer(mod),             // 15 pts
...
Anti-Corruption Layer              [##########] 15/15  index.ts=yes, internal.ts=yes
